...
open class GKPageScrollView: UIView {
    ...
    open override func layoutSubviews() {
        ...
        if self.mainTableView.frame.equalTo(self.bounds) {
            return
        }
    ...
    public func listScrollViewDidScroll(scrollView: UIScrollView) {
        ...
        if self.isScrollToOriginal || self.isScrollToCritical {
            return
        }
...
//        NSLog("isCriticalPoint = \(isCriticalPoint),isCeilPoint = \(isCeilPoint),isDisableMainScrollInCeil = \(isDisableMainScrollInCeil)", "")
        if self.isCriticalPoint {
            // 上滑到临界点后，固定父TableView的位置
            self.set(scrollView: scrollView, offset: self.criticalOffset)
            //同时设置mainScrollView不能滑动
            self.isMainCanScroll = false
            //子的tableView可以滑动
            self.isListCanScroll = true
        } else {
...
extension GKPageScrollView: UITableViewDataSource, UITableViewDelegate {
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return self.isLoaded ? 1 : 0 //只有一行
    }

    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath)  //普通的TableViewCell
    ...
    public func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        var height = self.frame.size.height == 0 ? GKPage_Screen_Height : self.frame.size.height
        height -= (self.isMainScrollDisabled ? self.headerHeight : self.ceilPointHeight) //这个cell的高度，看当前主cell是否可以滚动
        return height
    }
